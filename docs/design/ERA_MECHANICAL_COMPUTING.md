...
## 6. TUI 视觉表现 (Visual Design)

基于 Textual 框架的字符渲染风格设定，本章将采用“复古终端”与“蒸汽朋克”结合的视觉风格。
**注**: 本项目推荐使用 **Maple Mono NF** 或其他 Nerd Font 字体，以获得最佳的图标显示效果。

### 6.1 环境色调 (Palette)

- **主色调**:
  - **黄铜色 (Brass)**: `#B5A642` - 用于UI边框、重要机械结构。
  - **铁锈红 (Rust)**: `#8B4513` - 用于敌人、危险区域、废弃设施。
  - **蒸汽白 (Steam)**: `#F0F8FF` (带透明度/Dim) - 用于烟雾、高光。
  - **煤烟黑 (Soot)**: `#1A1110` - 用于背景底色，营造工业时代的厚重感。

### 6.2 地图元素 (Map Tiles)

利用 Nerd Font 图标与 Rich 样式定义地图图块，提升视觉丰富度：

| 元素 | 符号 (NF) | 备用符号 | 颜色/样式 | 说明 |
| :--- | :--- | :--- | :--- | :--- |
| **墙壁** | `` (Square) | `█` | `bronze` | 工厂的金属墙壁或砖墙 |
| **地面** | `` (Small Dot) | `·` | `dim grey` | 铺满油污的地面 |
| **齿轮装置** | `` (Gear) | `⚙` | `bold gold` | 场景中的动态装饰，偶有旋转动画 |
| **蒸汽管道** | `═` (Box Draw) | `│` | `copper` | 连接各个机器的管道 |
| **蒸汽喷口** | `` (Cloud) | `~` | `white` | 周期性喷出蒸汽，阻挡视线 |
| **门** | `` (Door) | `+` | `brown` | 机械闸门 |
| **宝箱** | `` (Package) | `?` | `bold yellow` | 隐藏的奖励 |

### 6.3 角色与实体 (Entities)

| 实体 | 符号 (NF) | 备用符号 | 颜色 | 视觉特征 |
| :--- | :--- | :--- | :--- | :--- |
| **玩家** | `` (Astronaut) | `@` | `bold cyan` | 象征“算界旅人”的探索者身份 |
| **巴贝奇** | `` (Teacher) | `B` | `bold gold` | 象征权威与核心地位 |
| **艾达** | `` (Code) | `L` | `bold purple` | 象征算法与逻辑 |
| **算盘兵** | `` (Calculator) | `A` | `yellow` | 象征机械计算的执行者 |
| **齿轮犬** | `` (Paw) | `d` | `orange` | 机械生物 |
| **蒸汽罐** | `` (Flask) | `O` | `red blink` | 易爆危险品 |
| **错误抄写员**| `` (Edit) | `S` | `dim white` | 象征过时的记录者 |

### 6.4 界面特效 (UI Effects)

- **CRT 滤镜模拟**:
  - 虽然是纯字符，但可以通过 Textual 的 CSS 样式给整个屏幕添加微弱的 `scanline` 效果（如果性能允许）。
- **机械运转动画**:
  - 状态栏或加载界面使用 `` `` 等不同齿轮图标切换，或使用 `⣾` `⣽` `⣻` `⢿` `⡿` `⣟` `⣯` `⣷` Braille 字符模拟旋转。
- **过热警告**:
  - 当玩家 EP 过低或处于“过热”状态时，屏幕边缘的边框颜色变为红色并呼吸闪烁。
